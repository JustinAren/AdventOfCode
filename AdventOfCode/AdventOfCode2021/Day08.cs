...
public readonly record struct Pattern(string[] InputSignals, string[] OutputSignals)
{
	public static Pattern Parse(string input)
	{
		...
		var result = new Pattern(splits[0].Split(' ', StringSplitOptions.RemoveEmptyEntries), splits[1].Split(' ', StringSplitOptions.RemoveEmptyEntries));
		return result;
	}

	public long CalculateOutputSignal()
	{
		var inputSignalList = this.InputSignals.Select(signal => new string(signal.OrderBy(c => c).ToArray())).ToList();
		...
		foreach (var outputSignal in this.OutputSignals.Select(signal => new string(signal.OrderBy(c => c).ToArray()))) outputBuilder.Append(outputBuilderDictionary[outputSignal]);
		var result = Int64.Parse(outputBuilder.ToString());
		return result;

		static bool Contains(string original, string substring) => substring.All(c => original.IndexOf(c) != -1);
	}
}