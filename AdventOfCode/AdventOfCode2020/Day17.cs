...
public class Day17 : Day<HashSet<(int X, int Y, int Z, int W)>>
{
	...
	private static HashSet<(int X, int Y, int Z, int W)> PerformCycle(HashSet<(int X, int Y, int Z, int W)> activeCubes, bool useW)
	{
	...
	private static HashSet<(int X, int Y, int Z, int W)> GetAllNeighbors(HashSet<(int X, int Y, int Z, int W)> activeCubes, bool useW)
	{
	...
	private static int GetActiveNeighborCount((int X, int Y, int Z, int W) cube, HashSet<(int X, int Y, int Z, int W)> activeCubes, bool useW)
	{
		var activeCount = 0;
			...
			for (var j = -1; j <= 1; j++)
			{
				for (var k = -1; k <= 1; k++)
				{
					...
					for (var l = wLowerBound; l <= wUpperBound; l++)
					{
						...
						if (activeCubes.Contains((cube.X + i, cube.Y + j, cube.Z + k, cube.W + l))) activeCount++;
					}
				}
			}
		}
