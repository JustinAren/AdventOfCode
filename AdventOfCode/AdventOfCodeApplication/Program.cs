...
public class Program
{
	public static void Main()
	{
		...
		while (true)
		{
			...
			if (!Int32.TryParse(yearString, out var year))
			{
			...
			if (!Int32.TryParse(dayString, out var dayNumber))
			{
			...
			if (!Int32.TryParse(problemString, out var problemNumber))
			{
			...
			{
				2015 => typeof(AdventOfCode2015.Day01).Assembly.GetType($"AdventOfCode{year:D4}.Day{dayNumber:D2}"),
				2020 => typeof(AdventOfCode2020.Day01).Assembly.GetType($"AdventOfCode{year:D4}.Day{dayNumber:D2}"),
				2021 => typeof(AdventOfCode2021.Day01).Assembly.GetType($"AdventOfCode{year:D4}.Day{dayNumber:D2}"),
				_ => null,
			};
			...
			if (day is null)
			{
				Console.WriteLine($"Implementation of AdventOfCode{year:D4}.Day{dayNumber:D2} cannot be created");
				continue;
			}
			...
			if (!performed)
			{
				Console.WriteLine($"Implementation of AdventOfCode{year:D4}.Day{dayNumber:D2} Problem {problemNumber} cannot be performed");
				continue;
			}

			Console.WriteLine($"Solution of Year {year:D4}, Day {dayNumber:D2}, Problem {problemNumber} is: {result}. Performed in {stopwatch.Elapsed:c}.");
